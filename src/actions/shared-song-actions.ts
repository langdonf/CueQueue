...
import type { CreateSongInput, SongItem } from "@/lib/types";
...
export async function sharedRemoveSongFromSetlist(
  ...
  await admin.rpc("recompact_setlist_positions", { p_setlist_id: setlistId });
...
export async function sharedReorderSongs(
  ...
  // Single RPC call handles reordering + timestamp update
  const { error } = await admin.rpc("reorder_setlist_songs_admin", {
